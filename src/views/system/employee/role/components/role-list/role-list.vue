<template>
  ...
  <div>
    <Row>
      <Col span="16" style="margin:20px 0;font-size: 15px;color: #95a5a6;">
        管理拥有当前角色权限的人员列表
      </Col>
      <Col span="8" style="margin:20px 0; text-align: right;">
      <Button @click="addEmployee" type="primary" v-privilege="'add-employee-role'">添加成员</Button>

      <Button @click="deleteEmployees()" style="margin-left:10px" type="error" v-privilege="'delete-employee-role-batch'">批量移除</Button>
      </Col>
    </Row>
    <!--Table 表格列表 start-->
    <Table :columns="columns" :data="tableData" :loading="isShowTablesLoading" @on-selection-change="selectChange"
      border ref="selection"></Table>
    <!--Table 表格列表 end-->
    <Row class="page" justify="end" type="flex">
      <Col order="2" span="24" style="text-align: right;margin-top:20px;">
      <Page :current="currentPage" :page-size="pageSize" :total="total" @on-change="changePage" show-elevator></Page>
      </Col>
    </Row>
    <!--modal 添加成员 start-->
    <Modal @on-ok="confirmPrepAddEmployees()" style="min-width:1800px" title="添加成员" v-model="isShowEmployeeModal" width="700">
      <Row class="shuttle-box">
        <!--Col 左侧用户列表 start-->
        <Col class="box" span="11">
        <Row>
          <Col class="title" span="24">用户列表</Col>
        </Row>
        <Row>
          <Col @dblclick.native="getCharge" class="no-scrollbar" id="goRight" span="24" style="height: 290px;overflow-y:scroll">
        ...
        <Icon @click.native="addPrepEmployees" size="30" style="line-height: 350px" type="md-arrow-round-forward"></Icon>
        </Col>
        <!--Col 转移按钮 end-->
        <!--Col 右侧预添加列表 start-->
        <Col class="box" span="11">
        <Row>
          ...
          <Col class="no-scrollbar" span="24" style="overflow-y:scroll;height: 290px;text-align: center">

  ...
  <div>
    ...
    >
      <Form :label-width="80" :model="updateItem" :rules="updateValidate" ref="updateRef">
        <FormItem label="社团名称" prop="positionName">
          <Input placeholder="请输入社团名称(必填)" v-model="updateItem.positionName"></Input>
        </FormItem>
        <FormItem label="社团描述" prop="remark">
          <Input
            :autosize="{minRows: 4}"
            :maxlength="200"
            placeholder="请输入社团描述(必填)"
            type="textarea"
    ...
    >
      <Form :label-width="80" :model="saveItem" :rules="saveValidate" ref="saveRef">
        <FormItem label="社团名称" prop="positionName">
          <Input :maxlength="30" placeholder="请输入社团名称(必填)" v-model="saveItem.positionName"></Input>
        </FormItem>
        <FormItem label="社团描述" prop="remark">
          <Input
            :autosize="{minRows: 4}"
  ...
  data() {
    return {
      ...
      updateValidate: {
        ...
        remark: [{ required: true, message: '请输入社团描述', trigger: 'blur' }]
      },
      saveValidate: {
        positionName: [
          { required: true, message: '请输入社团名称', trigger: 'blur' }
  ...
  methods: {
    ...
    async getPositionListPage() {
      try {
        ...
        let result = await positionApi.getPositionByPage(this.searchFrom);
